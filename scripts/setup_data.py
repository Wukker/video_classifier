...
Скачивает frame-level tfrecord файлы (partition=2/frame/validate, shard=1,10).
...
VALIDATE_DIR  = BASE_DIR / 'video' / 'frame_validate'
...
def main():
    print("=" * 55)
    print("  YT8M Dataset Setup (frame-level)")
    print("=" * 55)

    # 1. Создаём папки
    print("\n📂 Создаём папки...")
    VALIDATE_DIR.mkdir(parents=True, exist_ok=True)
    (BASE_DIR / 'frame_processed').mkdir(exist_ok=True)
    ...
    # 5. Скачиваем frame-level tfrecord (shard=1,10 → ~380 файлов, ~4 GB)
    print(f"\n🔄 Скачиваем frame-level tfrecord файлы (shard=1,10 ≈ 380 файлов, ~4 GB)...")
    print("   Это займёт 20-60 минут в зависимости от скорости интернета\n")

    env = os.environ.copy()
    env['partition'] = '2/frame/validate'
    env['mirror']    = 'eu'
    env['shard']     = '1,10'
    ...
    print(f"{'=' * 55}")
    print("  Следующий шаг: pipeline/01_parse_frame_level.ipynb")
