...
class YouTube8MFeatureExtractor(object):
  ...
  ** Note: OpenCV reverses the order of channels (i.e. orders channels as BGR
  ...
  def extract_audio_features(self, wav_path, n_mels=128):
    """Returns zeros for audio features.

    The classifier was trained on VGGish embeddings. Without the real VGGish
    model, passing zeros is safer than passing log-mel features with a
    mismatched distribution — the audio branch stays neutral instead of
    actively misleading the classifier.
    """
    return numpy.zeros((1, n_mels), dtype=numpy.float32)  # (1, 128)
